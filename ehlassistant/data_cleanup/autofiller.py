...
def main() -> None:
    ...
    id_list = rc.get_id({"taustainfo_complete": ["0"], "auto_status": ["3"]})
    #id_list = rc.get_id({"auto_status": ["3"]})
    #id_list = [465]
    id_list = order_sequence(id_list)
    for rc_id in id_list:
        print("### ",rc_id, " ###")
        record = rc.download(rc_id)
        upload_queue = {}
        upload_queue = {**upload_queue, **fix_diagnoses(rc_id, record)}
        for i in upload_queue:
            record[i] = upload_queue[i] #Update local copy
        upload_queue = {**upload_queue, **copy_auto(rc_id, record)}
        upload_queue["auto_status"] = "4"
        print(upload_queue)
        rc.upload(data=upload_queue, redcap_id=rc_id, overwrite=True)
        #break

def fix_diagnoses(rc_id: int, record: dict) -> None:
    ...
    for i in range(1,25):
        ...
        if record[auto_key] == "0":
            if diagnosis_set & diagnosis_codes[key]:
                ...
                print(f"Updating - {rc_id} - {descriptors[key]}")
    for i in range(1,9):
...
def copy_auto(rc_id: int, record: dict) -> None:
